...
// computes |z|
    ...
    // Added by Talbi Rania
    inline mpz_class
    mpz_class_urandomm(gmp_randstate_t state, const mpz_class n)
    {
        mpz_class rop;
    ...
    // Added by Rania Talbi to make  the generation of random number of a specific size faster.

    inline mpz_class
    mpz_class_get_z_bits (mpz_class nbits)
    {
        gmp_randclass X (gmp_randinit_default);
        X.seed(time(0));
        return X.get_z_bits(nbits);
    }
...
// Added by Rania Talbi

inline mpz_class
mpz_class_sqrt(const mpz_class &x)
{
    mpz_class a;
    mpz_sqrt(a.get_mpz_t(), x.get_mpz_t());
    return a;
}
